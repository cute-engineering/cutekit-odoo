...
def _(args: cli.Args):
    ...
    for repo in cache.registry.project.externDirs:
        branches += list(
            filter(
                lambda b: bool(b),
                shell.popen("git", "-C", cache.rootdir / repo, "branch", "-r").split(),
            )
        )
...
def _(args: cli.Args):
    ...
    select = args.consumeOpt("select", False)
    cache = utils.Cache(model.Registry.use(args))
    ...
    if not select:
        ...
        for repo in directories:
            ...
            shell.exec("git", "-C", cache.rootdir / const.EXTERN_DIR / "odoo" / repo, "rebase", f"origin/{target}")
...
def _(args: cli.Args):
    ...
    for repo in cache.registry.project.externDirs:
        ...
        if branch[0] != "dev" and repo:
            if branch[1] == "master" or repo != os.path.join(
                const.EXTERN_DIR, "odoo", "upgrade"
            ):
        ...
        try:
            ...
            shell.exec(
                "git", "-C", cache.rootdir / repo, "commit", "--amend", "--no-edit"
            )
        except RuntimeError:
