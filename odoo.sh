...
if [ "$1" == "tools" -a "$2" == "nuke" ]; then
    rm -rf $dir/.cutekit/tools $dir/.cutekit/venv
    exit 0
fi

if [ ! -f $dir/.cutekit/tools/ready ]; then
    ...
    if [ ! -d .cutekit/venv ]; then
        ...
        $CUTEKIT_PYTHON -m venv $dir/.cutekit/venv
    fi
    source $dir/.cutekit/venv/bin/activate

    echo "Downloading CuteKit..."
    if [ ! -d .cutekit/tools/cutekit ]; then
        git clone --depth 1 https://github.com/cute-engineering/cutekit $dir/.cutekit/tools/cutekit --branch "$CUTEKIT_VERSION"
    else
        echo "CuteKit already downloaded."
    fi

    echo "Installing Tools..."
    $CUTEKIT_PYTHON -m pip install -e $dir/.cutekit/tools/cutekit

    echo "Installing plugins requirements..."
    if [ -f "$dir/meta/plugins/requirements.txt" ]; then
        echo "Root plugin requirements found."
        $CUTEKIT_PYTHON -m pip install -r $dir/meta/plugins/requirements.txt
    fi

    for extern in $dir/meta/externs/*; do
    ...
    if is_ubuntu; then
        ...
        sudo $dir/meta/scripts/setup-ubuntu.sh
    fi

    touch $dir/.cutekit/tools/ready
    echo "Done!"
fi
...
source $dir/.cutekit/venv/bin/activate
export PATH="$PATH:$dir/.cutekit/venv/bin"
